...
class TestInviteUser:
    ...
    async def test_happy_path(self, adapter, mock_client):
        ...
        tid = uuid.uuid4()
        result = await adapter.invite_user(email="alice@test.com", tenant_id=tid, role_names=["member"])
        assert result == Ok(None)
        mock_client.invite_user.assert_awaited_once_with("alice@test.com", str(tid), ["member"])

    @pytest.mark.anyio
    async def test_http_error_returns_sync_error(self, adapter, mock_client):
        mock_client.invite_user = AsyncMock(side_effect=_make_http_status_error(400))
        result = await adapter.invite_user(email="a@b.com", tenant_id=uuid.uuid4(), role_names=["member"])
        assert result.is_error()
        assert isinstance(result.error, SyncError)
        assert result.error.operation == "invite_user"

    @pytest.mark.anyio
    async def test_request_error_returns_sync_error(self, adapter, mock_client):
        mock_client.invite_user = AsyncMock(side_effect=httpx.RequestError("Connection refused"))
        result = await adapter.invite_user(email="a@b.com", tenant_id=uuid.uuid4(), role_names=["member"])
        assert result.is_error()
        assert isinstance(result.error, SyncError)
    ...
    async def test_error_contains_context(self, adapter, mock_client):
        tid = uuid.uuid4()
        mock_client.invite_user = AsyncMock(side_effect=_make_http_status_error(500))
        result = await adapter.invite_user(email="a@b.com", tenant_id=tid, role_names=["member"])
        assert result.error.context["email"] == "a@b.com"
        assert result.error.context["tenant_id"] == str(tid)
    ...
    async def test_logs_warning_on_error(self, adapter, mock_client):
        mock_client.invite_user = AsyncMock(side_effect=_make_http_status_error(500))
        with patch("app.services.adapters.descope.logger") as mock_logger:
            await adapter.invite_user(email="a@b.com", tenant_id=uuid.uuid4(), role_names=["member"])
            mock_logger.warning.assert_called_once()


# ---------------------------------------------------------------------------
# remove_user_from_tenant (Story 2.3)
# ---------------------------------------------------------------------------


class TestRemoveUserFromTenantAdapter:
    @pytest.mark.anyio
    async def test_happy_path(self, adapter, mock_client):
        mock_client.remove_user_from_tenant = AsyncMock()
        uid = uuid.uuid4()
        tid = uuid.uuid4()
        result = await adapter.remove_user_from_tenant(user_id=uid, tenant_id=tid)
        assert result == Ok(None)
        mock_client.resolve_login_id.assert_awaited_once_with(str(uid))
        mock_client.remove_user_from_tenant.assert_awaited_once_with("login-id-123", str(tid))

    @pytest.mark.anyio
    async def test_404_skips(self, adapter, mock_client):
        mock_client.resolve_login_id = AsyncMock(side_effect=_make_http_status_error(404))
        result = await adapter.remove_user_from_tenant(user_id=uuid.uuid4(), tenant_id=uuid.uuid4())
        assert result == Ok(None)
    ...
    async def test_http_error_returns_sync_error(self, adapter, mock_client):
        ...
        assert result.is_error()
        assert isinstance(result.error, SyncError)
        assert result.error.operation == "remove_user_from_tenant"

    @pytest.mark.anyio
    async def test_request_error_returns_sync_error(self, adapter, mock_client):
        mock_client.resolve_login_id = AsyncMock(side_effect=httpx.RequestError("timeout"))
        result = await adapter.remove_user_from_tenant(user_id=uuid.uuid4(), tenant_id=uuid.uuid4())
        assert result.is_error()
        assert isinstance(result.error, SyncError)
    ...
    @pytest.mark.anyio
    async def test_value_error_returns_sync_error(self, adapter, mock_client):
        mock_client.resolve_login_id = AsyncMock(side_effect=ValueError("bad id"))
        result = await adapter.remove_user_from_tenant(user_id=uuid.uuid4(), tenant_id=uuid.uuid4())
        assert result.is_error()
        assert isinstance(result.error, SyncError)
...
# ---------------------------------------------------------------------------
# remove_role_assignment (Story 2.3)
# ---------------------------------------------------------------------------


class TestRemoveRoleAssignment:
    @pytest.mark.anyio
    async def test_happy_path(self, adapter, mock_client):
        mock_client.remove_roles = AsyncMock()
        uid = uuid.uuid4()
        tid = uuid.uuid4()
        rid = uuid.uuid4()
        result = await adapter.remove_role_assignment(
            user_id=uid,
            tenant_id=tid,
            role_id=rid,
            role_name="admin",
        ...
        assert result == Ok(None)
        mock_client.resolve_login_id.assert_awaited_once_with(str(uid))
        mock_client.remove_roles.assert_awaited_once_with("login-id-123", str(tid), ["admin"])
    ...
    async def test_http_error_returns_sync_error(self, adapter, mock_client):
        ...
        assert result.is_error()
        assert isinstance(result.error, SyncError)
        assert result.error.operation == "remove_role_assignment"
    ...
    async def test_logs_warning_on_error(self, adapter, mock_client):
        mock_client.resolve_login_id = AsyncMock(side_effect=_make_http_status_error(500))
        with patch("app.services.adapters.descope.logger") as mock_logger:
            await adapter.remove_role_assignment(
                user_id=uuid.uuid4(),
                tenant_id=uuid.uuid4(),
                role_id=uuid.uuid4(),
                role_name="admin",
