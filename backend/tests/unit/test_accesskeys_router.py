...
async def test_list_keys_as_admin(mock_validate, mock_factory, client):
    ...
    mock_client.search_access_keys.return_value = [
...
async def test_create_key(mock_validate, mock_factory, client):
    ...
    mock_client.create_access_key.assert_called_once_with(
        name="My API Key", tenant_id="tenant-abc", expire_time=None, role_names=None
    )
...
async def test_create_key_with_options(mock_validate, mock_factory, client):
    ...
    assert response.status_code == 200
    mock_client.create_access_key.assert_called_once_with(
        name="Scoped Key", tenant_id="tenant-abc", expire_time=1700000000, role_names=["viewer"]
    )
...
async def test_get_key(mock_validate, mock_factory, client):
    mock_validate.return_value = ADMIN_CLAIMS
    mock_client = AsyncMock()
    mock_client.load_access_key.return_value = {"id": "key123", "name": "Test", "status": "active"}
    mock_factory.return_value = mock_client
