"""Unit tests for DescopeSyncAdapter (AC-2.1.4).
...
- Unimplemented methods return Ok(None)
...
@pytest.mark.anyio
class TestUnimplementedMethodsReturnOk:
    async def test_sync_role(self, adapter):
        result = await adapter.sync_role(role_id=uuid.uuid4(), data={})
        assert result == Ok(None)

    async def test_sync_permission(self, adapter):
        result = await adapter.sync_permission(permission_id=uuid.uuid4(), data={})
        assert result == Ok(None)

    async def test_sync_tenant(self, adapter):
        result = await adapter.sync_tenant(tenant_id=uuid.uuid4(), data={})
        assert result == Ok(None)

    async def test_sync_role_assignment(self, adapter):
        result = await adapter.sync_role_assignment(user_id=uuid.uuid4(), tenant_id=uuid.uuid4(), role_id=uuid.uuid4())
        assert result == Ok(None)
